...
export const getStarColor = (temp: number): string => {
  ...
  for (let i = 0; i < colorTemperatureEquivalencies.length; i++) {
    if (
      temp <= colorTemperatureEquivalencies[i]?.temp &&
      temp > colorTemperatureEquivalencies[i + 1]?.temp
    ) {
...
export const getStarColorAsNumber = (temp: number): number =>
  parseInt(getStarColor(temp).substring(1), 16);
