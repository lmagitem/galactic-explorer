...
import {
  ...
  GalacticMapDivisionLevelDTO,
} from "../../models/galactic-map-division-level";
...
export const convertGalaxyFromDTO = (dto: GalaxyDTO): Galaxy => {
  ...
  result.divisionLevels = dto.division_levels.map((level) =>
    convertGalacticDivisionLevelFromDTO(level),
  );
...
export const convertGalacticDivisionLevelFromDTO = (
  dto: GalacticMapDivisionLevelDTO,
): GalacticMapDivisionLevel => ({
  level: dto.level,
